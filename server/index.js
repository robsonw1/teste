...
app.post('/api/print', express.json({ type: '*/*' }), async (req, res) => {
  try {
    ...
    if (!webhookUrl) {
      console.error('❌ /api/print: PRINT_WEBHOOK_URL não configurado');
      return res.status(400).json({ error: 'PRINT_WEBHOOK_URL not configured on server' });
    }
    ...
    // Salvar último pedido para debug (não bloquear)
    try {
      ...
      console.log('💾 Pedido salvo em:', lastPath);
    } catch (e) {
      console.warn('⚠️ Falha ao salvar last-print.json:', e.message);
    }
    ...
    // Outbound log line for easy correlation in hosting logs
    try {
      const tsOut = new Date().toISOString();
      console.log(`HTTP ${tsOut} OUT POST ${webhookUrl}`);
    } catch (e) { /* ignore */ }
    ...
    // Save a preview of upstream response for debugging (non-blocking)
    try {
      const respPath = path.join(LOGO_DIR, 'last-print-response.txt');
      await fs.promises.writeFile(respPath, `status=${upstream.status}\nelapsed=${elapsed}ms\n\n${String(text || '').slice(0,2000)}`, 'utf8');
      console.log('� Upstream response preview saved to', respPath);
    } catch (e) {
      console.warn('⚠️ Could not save upstream response preview:', e && e.message ? e.message : e);
    }

    console.log('�📥 Resposta do n8n:');
    console.log('   Status:', upstream.status);
    console.log('   Tempo:', elapsed, 'ms');
    console.log('   Body preview:', String(text || '').slice(0, 500));

    // If the upstream returned an HTML page (index.html or similar), treat as an error
    const lower = String(text || '').toLowerCase();
    if (lower.includes('<!doctype') || lower.includes('<html') || (upstream.headers && String(upstream.headers.get('content-type') || '').toLowerCase().includes('text/html'))) {
      console.error('❌ Print webhook responded with HTML/page instead of webhook JSON — likely wrong URL or routing');
      return res.status(502).json({ 
        error: 'Print webhook returned HTML (not a webhook response)',
    ...
    if (!upstream.ok) {
      ...
      return res.status(502).json({ 
        error: 'Print webhook error', 
        status: upstream.status, 
        detail: text || 'Empty response'
      });
    }

    // Verificar se a resposta não está vazia
    if (!text || !String(text).trim()) {
      console.warn('⚠️ n8n retornou resposta vazia');
      return res.status(502).json({ 
        error: 'Empty response from print webhook', 
        status: upstream.status 
      });
    }
    ...
    try {
      ...
      // Se não for JSON mas retornou texto válido (não-HTML), aceitar
      console.log('✅ Impressão enviada (resposta não-JSON)');
      return res.send(text);
    }

  } catch (err) {
    console.error('❌ Erro crítico no proxy /api/print:', err);
